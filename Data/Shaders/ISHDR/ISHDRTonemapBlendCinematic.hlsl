...
#ifdef SHADERTOOLS
#include "../SimpleGraphicsSuite/Settings.hlsli"
...
// The game's own tonemap/grading formula, unmodified - used in place of the
// ENB-style grading below when SGS_PostProcessingEnabled is off.
float3 Vanilla(float3 a_color, float2 a_uv, float2 a_scaledUV)
{
	float4 color = float4(a_color, 0.0);

	bool scaleBloom = (0.5 <= Params01[0].x);
	float3 bloom = TextureBloom.Sample(TextureBloomSampler, scaleBloom ? a_scaledUV : a_uv).rgb;

	float2 middlegray = TextureAdaptation.Sample(TextureAdaptationSampler, a_uv).xy;
...
// ACES filmic fit by Krzysztof Narkowicz (2015), the standard simple
// approximation of the full ACES reference tonemapper.
...
{
	float2 vig2 = uv * (1.0 - uv);
	float  vig  = saturate(pow(vig2.x * vig2.y * 15.0, 0.25));
...
PS_OUTPUT main(PS_INPUT input)
{
	...
	if (SGS_UpscalingEnabled > 0.5) {
		// The scene renders under native resolution (Upscaling module drives
		// dynamicResolutionWidthRatio/HeightRatio) into a sub-rect of this
		// same full-size buffer - EASU reconstructs that sub-rect back up to
	...
	if (SGS_PostProcessingEnabled > 0.5) {
		...
		// Saturation as ICtCp chroma scaling (same approach FrostbyteTonemap
		// below already uses) instead of a plain lerp-away-from-luma - that
		// lerp can push a channel below zero once saturation exceeds 1, which
		// the log-domain contrast step right after this then crushes toward
		// black. Scaling chroma only, luma untouched, can't do that.
		{
			float3 ictcpColor = rgb2ictcp(Color);
			ictcpColor.yz *= IN.UIHCG_Saturation;
			Color = zerolim(ictcp2rgb(ictcpColor));
		}
		...
		//----------------------------------- Channel Crosstalk -----------------------------------//
			...
			ColRatio = pow(ColRatio, SETTING_UICT_Saturation);
			Color	 = lerp(Color, ColRatio * MaxColor, SETTING_UICT_Weight);
		#endif
