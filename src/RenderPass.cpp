...
namespace RenderPass
{
    ...
    bool Target::Ensure(std::uint32_t a_width, std::uint32_t a_height, REX::W32::DXGI_FORMAT a_format)
    {
        if (texture && width == a_width && height == a_height)
            return true;
        ...
        if (FAILED(device->CreateTexture2D(&desc, nullptr, &texture))) {
            logger::error("RenderPass: couldn't create a {}x{} target texture", a_width, a_height);
            return false;
        }

        REX::W32::D3D11_RENDER_TARGET_VIEW_DESC rtvDesc{};
        rtvDesc.format = a_format;
        rtvDesc.viewDimension = REX::W32::D3D11_RTV_DIMENSION_TEXTURE2D;
        if (FAILED(device->CreateRenderTargetView(
                static_cast<REX::W32::ID3D11Resource*>(texture), &rtvDesc, &rtv))) {
            logger::error("RenderPass: couldn't create a render target view");
            Release();
            return false;
        }
        ...
        if (FAILED(device->CreateShaderResourceView(
                static_cast<REX::W32::ID3D11Resource*>(texture), &srvDesc, &srv))) {
            logger::error("RenderPass: couldn't create a shader resource view");
            Release();
            return false;
        }
